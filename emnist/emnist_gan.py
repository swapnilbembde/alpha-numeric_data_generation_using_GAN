...
if not os.path.isdir('emnist_results'):
    os.mkdir('emnist_results')
if not os.path.isdir('emnist_results/images'):
    os.mkdir('emnist_results/images')


def save_images(num_epoch, show = False, save = False, path = 'result.png'):
    z_ = torch.randn((7*7, 100))
...
train_loader = torch.utils.data.DataLoader(
    datasets.EMNIST('data',split = 'bymerge', train=True, download=False, transform=transform),
    batch_size=128)
...
for epoch in tqdm(range(epochs)):
    ...
    p = 'emnist_results/images/' + str(epoch + 1)+ '.png'
...
loss_plots(train_hist, save=True, path='emnist_results/EMNIST_GAN_train_hist.png')
torch.save(G.state_dict(), "emnist_results/generator_param.pkl")
torch.save(D.state_dict(), "emnist_results/discriminator_param.pkl")
with open('emnist_results/train_hist.pkl', 'wb') as f:
    pickle.dump(train_hist, f)

# creating gif file     
images = []
for i in range(epochs):
    img_name = 'emnist_results/images/' + str(i + 1) + '.png'
    images.append(imageio.imread(img_name))
imageio.mimsave('emnist_results/gif_file.gif', images, fps=5)
